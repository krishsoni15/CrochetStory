...
export default function ProductCardShop({ product, onEdit, onDelete, onProductClick, onOrderClick }) {
  ...
  return (
    ...
    >
      {/* Admin Edit/Delete Buttons - Visible on mobile, on right side on hover */}
      {isAdmin && (
        <div className="absolute top-2 right-2 flex gap-1.5 z-30 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity duration-200">
          <motion.button
            onClick={(e) => {
              e.preventDefault();
              e.stopPropagation();
      ...
      <div className="p-4 flex flex-col flex-grow bg-gradient-to-b from-white to-gray-50/30 min-h-[180px]">
        ...
        <div className="mt-auto flex items-center justify-between gap-2">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            className="flex-1 bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white px-3 py-2 rounded-lg font-semibold text-xs shadow-md hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-1.5 touch-manipulation"
          >
